...
static cl::opt<bool> IntervalRangeDebug("intervalrange-debug", cl::Hidden,
...
bool isSupported(Instruction &i) {
    return (isa<PHINode>(i) || isa<BinaryOperator>(i) || isa<AllocaInst>(i)
            || isa<LoadInst>(i) || isa<CallInst>(i) || isa<SelectInst>(i)
            || isa<ICmpInst>(i));
}
 ...
 * instructions.
 */
Interval getInterval(Value *V, StateMap state) {
    Interval result;
...
bool IntervalRangeAnalysis::runOnFunction(Function &F) {
    ...
    //add existing constants to a set so that it can be used for widening
    std::set<int> constSet;
    constSet.insert(interval::minf);
    constSet.insert(interval::pinf);
    
    for (BasicBlock &bb : F) {
        for (Instruction &i : bb) {
            for (auto op = i.operands().begin(); op < i.operands().end();
                 op++) {
                if (ConstantInt *CI = dyn_cast<ConstantInt>(op->get())) {
                    constSet.insert(CI->getSExtValue());
                }
            }
        }
    }
    ...
    if (IntervalRangeDebug) {
        ...
        errs() << "DEBUG: initial constant set\n";
        for (int v : constSet) {
            errs() << "-->" << v << "\n";
        }
    }
    ...
    while (!w.empty()) {
        ...
        if (auto *phi = dyn_cast<PHINode>(i)) {
            ...
            for (int idx = 0; idx < phi->getNumIncomingValues(); idx++) {
                ...
                if (IntervalRangeDebug) {
                    ...
                    errs()
                    << str(
                           getInterval(phi->getIncomingValue(idx),
                                       state));
                    errs() << ") = " << str(newlub) << "\n";
                }
        ...
        if (old != current) {
            //widening
            int lo, hi;
            if (interval::lower(old) == interval::pinf) {
                lo = interval::lower(current);
            } else {
                if (interval::lower(old) <= interval::lower(current)) {
                    lo = interval::lower(old);
                } else {
                    std::set<int>::iterator it = constSet.lower_bound(interval::lower(current));
                    if (*it != interval::minf) it--;
                    lo = *it;
                }
            }
            if (interval::upper(old) == interval::minf) {
                hi = interval::upper(current);
            } else if (interval::upper(old) >= interval::upper(current)) {
                hi =  interval::upper(old);
            } else {
                hi = *constSet.upper_bound((interval::upper(current) - 1));
            }
            current = interval::make(lo, hi);
            if (IntervalRangeDebug) {
                errs() << "DEBUG: analyzing after widening " << *i << "\n";
                errs() << "--> new value  =" << str(current) << "\n";
            }
